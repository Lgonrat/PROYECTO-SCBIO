...
def check_click(event, x, y, flags, param):
    global current_image, game_started, scoreCPU, scorePLAYER
    ...
    click_area_x_reglas2 = [106, 600, 108, 600]  # Coordenadas en X
    click_area_y_reglas2 = [1000, 1000, 800, 800]  # Coordenadas en Y
        ...
        if current_image == "presentation":
            if min(click_area_x_start) <= x <= max(click_area_x_start) and min(click_area_y_start) <= y <= max(click_area_y_start):  # Coordenadas para Start
                ...
                for i in range(3, 0, -1):
                    countdown_img = cv2.imread(f"Cuenta{i}.jpeg")
                    cv2.imshow('Webcam', countdown_img)
                    cv2.waitKey(1000)  # Mostrar la imagen de la cuenta regresiva durante 1 segundo

                # Después de que el contador ha terminado, establecer game_started en True para comenzar el juego
                game_started = True
                scoreCPU = 0
                scorePLAYER = 0
...
with mp_hands.Hands(model_complexity=0, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    ...
    while cap.isOpened():
        ...
        if game_started:
            ...
            countdown_text = ""
            # Duración en segundos para mostrar las opciones
            OPTIONS_DISPLAY_TIME = 3 if elapsed_time > countdown else 6  # 6 segundos para la primera elección, 3 segundos para el resto
            ...
            if elapsed_time < countdown:
                ...
                if elapsed_time > countdown + (6 if not options_displayed else OPTIONS_DISPLAY_TIME):
            ...
            countdown_position = (combined_width // 2 - 15, combined_height // 2 - 50)
